...
locals {
  ...
  uptimerobot_whitelist_cidrs = distinct(compact([
  ...
  uptimerobot_whitelist_cidrs_by_region = {
    for region in local.uptimerobot_regions : region => distinct(compact([
      for entry in local.uptimerobot_prefix_entries : entry.cidr
  ...
  uptimerobot_whitelist_ipv4_cidrs = [
    for cidr in local.uptimerobot_whitelist_cidrs : cidr
    if can(regex("\\.", cidr))
  ]

  uptimerobot_whitelist_ipv6_cidrs = [
    for cidr in local.uptimerobot_whitelist_cidrs : cidr
    if can(regex(":", cidr))
  ]

  uptimerobot_whitelist_ipv4_cidrs_by_region = {
    for region, cidrs in local.uptimerobot_whitelist_cidrs_by_region : region => [
